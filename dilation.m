%%%%%%%%%%%%%%% Function erosion %%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Purpose:
%         Implement dilation with certain structuring element to an image
% 
% Input Variable:
...
%         3. If one of the pixels on the extracted image has the same value as the 
%             corresponding position on the structuring element, set the origin of the 
%             input image to be black.
%        
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% END  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function output = dilation(input_image,structure)
    ...
    for i = 1+floor(x/2):rows-floor(x/2)
        for j = 1+floor(y/2):col-floor(y/2)
           ...
           if (min(temp(pos)) == 0) % if the position of the image pixel is black
               output(i, j) = 0;
           else
               output(i, j) = 255;
           end
